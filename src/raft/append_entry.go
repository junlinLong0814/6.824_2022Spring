...
func (rf *Raft) appendEntriesTricker() {
	for !rf.killed() {
		...
		for i := 0; i < len(rf.peers); i++ {
			...
			//check whether to send snapshot
			//when nextIndex[peer] <= leader.log[0].index
			//send sanpshot
			if rf.nextIndex[i] <= rf.logs[0].Index {
				go rf.CallSendInstallSnapshot(i, rf.currentTerm, rf.me,
					rf.persister.ReadSnapshot(),
					rf.logs[0].Index, rf.logs[0].Term)
			} else {
...
func (rf *Raft) callForAppend(peer, currentTerm, me, PrevLogIndex, PrevLogTerm, LeaderCommit int, logs []LogEntry) {
	...
	if ok {
		...
		if !reply.Success {
			...
			//SnapInfo("[%d] firstLog[%d],lastLog[%d],oldCommit[%d],newCommit[%d]\n",
			//	rf.me, rf.logs[0].Index, rf.logs[len(rf.logs)-1].Index, rf.commitIndex, newCommitindex)
...
func (rf *Raft) AppendEntries(args *appendArgs, reply *appendReply) {
	...
	if args.PrevLogIndex < rf.logs[0].Index {
		...
		SnapInfo("[%d] firstLog[%d],lastLog[%d],oldCommit[%d],newCommit[%d],args.LeaderCommit[%d]\n",
			rf.me, rf.logs[0].Index, rf.logs[len(rf.logs)-1].Index, commitIdxBak, rf.commitIndex, args.LeaderCommit)
